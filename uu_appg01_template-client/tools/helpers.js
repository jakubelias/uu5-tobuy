...
function getWebpackConfig(options) {
  ...
  getProjectPackageList(src).forEach(pack => {
    aliases[pack.moduleName] = path.resolve(src, pack.entryPoint);
  });
      ...
      if (Object.keys(mappingsJson || {}).length > 0) {
        ...
        app.use((req, res, next) => {
          if (req.method == "GET" && (!opts.appAssetsRelativeUrlPath || !req.url.startsWith("/" + opts.appAssetsRelativeUrlPath))) {
            ...
            var filePath = targetUc ? path.resolve(outputAbsPath, targetUc.realization||"") : null;
            if (!targetUc || (expectsHtml && (targetUc.type !== "VUC" || !fs.existsSync(filePath)))) {
              filePath = null;
              ...
              if (opts.appAssetsRelativeUrlPath || urlPath.indexOf(".") === -1) {
                targetUc = ucMap["defaultVuc"];
                filePath = targetUc ? path.resolve(outputAbsPath, targetUc.realization||"") : null;
              }
            }
            if (filePath && fs.existsSync(filePath) && fs.statSync(filePath).isFile()) return res.sendFile(targetUc.realization, { root: outputAbsPath});
          }
          next();
        });
      }
  ...
  module.exports = require("__project__/${noRelativeName}");`;
    ...
    fs.utimesSync(entryFilePath, timeInPast, timeInPast);
...
function getSystemJSLoaderHtmlSnippet(opts) {
  ...
  var browserUuEnvironment = opts.browserEnvironmentConfig;
  var environmentSnippet = "";
  if (browserUuEnvironment && (typeof browserUuEnvironment == "string" || typeof browserUuEnvironment == "object")) {
    var hasDirectUuEnvironment = typeof browserUuEnvironment != "string";
    if (hasDirectUuEnvironment) {
      environmentSnippet = `\nSystemJS.set("UU_ENVIRONMENT", SystemJS.newModule({"__useDefault": true, "default": ${JSON.stringify(browserUuEnvironment)}}));`
    } else {
      // NOTE Registering "meta" via System.config must be done at the same time as "paths" or later for given key.
      var metaEnv = { loader: "systemjs-plugin-json" };
      systemJsSettings.paths = systemJsSettings.paths || {};
      if (!systemJsSettings.paths["systemjs-plugin-json"]) {
        systemJsSettings.paths["systemjs-plugin-json"] = "https://cdnjs.cloudflare.com/ajax/libs/systemjs-plugin-json/0.3.0/json" + (opts.minify ? ".min" : "") + ".js"; // FIXME CDN
      }
      if (!browserUuEnvironment.match(/^[a-zA-Z0-9\\-_]+:/) && !browserUuEnvironment.match(/^\//)) { // URL is relative
        environmentSnippet = `
var appBase = document.querySelector("base").getAttribute("data-uu-app-base") || "";
SystemJS.config({ paths: { UU_ENVIRONMENT: appBase + ${JSON.stringify(browserUuEnvironment)} }, meta: { UU_ENVIRONMENT: ${JSON.stringify(metaEnv)} } });`;
      } else {
        systemJsSettings.paths["UU_ENVIRONMENT"] = browserUuEnvironment;
        systemJsSettings.meta = systemJsSettings.meta || {};
        systemJsSettings.meta["UU_ENVIRONMENT"] = metaEnv;
      }
      environmentSnippet += `\nSystemJS.import("UU_ENVIRONMENT");`; // start loading it ASAP, because it'll be needed (as it was defined to be used in our options...)
    }
  }
...
function getLoaderSettings(opts) {
  ...
  deps.forEach(depName => {
    ...
    if (typeof dep.baseUri !== "string") return; // skip special modules ("module", "UU_ENVIRONMENT")
...
function getMappingsJson() {
  var filePath = "../server/config/mappings.json";
